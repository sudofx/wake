...
<section class="dashboard-kpis">${card(s.version,'Durable cycles','Accepted state advances')}${card(acceptanceRate+'%','Acceptance rate',acceptedCount+' of '+completed.length+' completed wakes')}${card(handoffRate+'%','Obligation handoff',inheritedFulfilled.length+' cross-invocation fulfillments')}${card(requestsPerAccepted,'Requests / accepted','Recorded HTTP attempts ÷ accepted wakes')}${card(fallbackWakes,'Fallback wakes','More than one provider attempt')}${card(medianLatency===null?'—':medianLatency+'ms','Median provider latency','Known completed model attempts')}${card(revisedBeliefs,'Belief actions',activeBeliefs.length+' active · '+retractedBeliefs.length+' retracted')}${card(overdue,'Overdue obligations','Open commitments at or past due cycle')}</section></div>
  ...
  function blog(selected='') {
    if(selected.startsWith('topic:')){
      ...
      $('blog-content').innerHTML='<p class="topic-filter-note">Showing <strong>'+esc(topic==='reflection'?'reflection':topicLabel(topic).toLowerCase())+'</strong> · <a href="#blog">show all</a></p>'+(filtered.length?'<div class="blog-grid">'+filtered.map((post,index)=>{const invocation=s.invocations[post.created_by];const reflection=postTopic(post)==='reflection';return '<details class="blog-card record-panel'+(reflection?' blog-card-reflection':'')+'"'+(index<2?' open':'')+'><summary><div class="record-panel-meta"><time>'+esc(fmt(invocation.time))+'</time>'+topicTag(postTopic(post),'blog',postTopic(post)==='reflection'?'reflection':topicLabel(postTopic(post)))+'</div><h2>'+esc(post.title)+'</h2></summary><div class="record-panel-body"><p>'+esc(post.lede)+'</p>'+(post.lens?'<blockquote>'+esc(post.lens)+'</blockquote>':'')+'<a class="text-link" href="#blog/'+encodeURIComponent(post.id)+'">Read Bob’s note →</a></div></details>';}).join('')+'</div>':'<p class="empty">No posts match this topic.</p>');
      return;
    }
    if(selected){
    ...
    $('blog-content').innerHTML=posts.length?'<div class="blog-grid">'+posts.map((post,index)=>{const invocation=s.invocations[post.created_by];const reflection=(Number(post.created_version)%10===0)&&(/reflection/i.test(String(post.id))||/reflection/i.test(String(post.title)));return '<details class="blog-card record-panel'+(reflection?' blog-card-reflection':'')+'"'+(index<2?' open':'')+'><summary><div class="record-panel-meta"><time>'+esc(fmt(invocation.time))+'</time>'+topicTag(postTopic(post),'blog',postTopic(post)==='reflection'?'reflection':topicLabel(postTopic(post)))+'</div><h2>'+esc(post.title)+'</h2></summary><div class="record-panel-body"><p>'+esc(post.lede)+'</p>'+(post.lens?'<blockquote>'+esc(post.lens)+'</blockquote>':'')+'<a class="text-link" href="#blog/'+encodeURIComponent(post.id)+'">Read Bob’s note →</a></div></details>';}).join('')+'</div><p class="blog-disclosure">Bob is the public translation layer. Underneath, WAKE✳︎ is a sequence of fresh model calls working from a durable, auditable record—not a persistent person or experiencing self.</p>':'<div class="empty blog-empty"><strong>Bob has nothing worth posting yet.</strong><br>The journal still records every wake. The blog waits for something genuinely interesting.</div>';
  }
