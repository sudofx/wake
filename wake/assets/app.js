...
(() => {
  ...
  function journal() {
    ...
    $('entries').innerHTML=entries.slice(0,journalLimit).map((j,index) => {
      ...
      return `<details class="entry record-panel" id="cycle-${j.cycle}"${index<2?' open':''}><summary><div class="record-panel-meta"><span class="cycle">WAKE✳︎ ${String(j.cycle).padStart(3,'0')}</span>${badge(i.provider==='fixture'?'simulated':'accepted',i.provider==='fixture'?'SIMULATED':'ACCEPTED')}${journalTopics(j).map(id=>topicTag(id,'journal')).join('')}<time datetime="${esc(i.time)}">${esc(fmt(i.time))}</time></div><h3>${esc(j.title)}</h3></summary><div class="record-panel-body"><p>${esc(j.summary)}</p><div class="entry-bottom"><span>${esc(i.provider)} / ${esc(i.model)}</span><span>${actions.length} recorded change${actions.length===1?'':'s'}</span></div><details><summary>Open the lab notes →</summary>${actions.map(a=>`<div class="decision"><strong>${esc(a.type)} / ${esc(a.id)}</strong><p>${esc(a.statement||a.task||a.status)}</p><p>${esc(a.reason)}</p>${refs(a.evidence)}</div>`).join('') || '<p>No state changes proposed.</p>'}<a class="subtle" href="#history/${encodeURIComponent(j.invocation)}">Full invocation & decision →</a></details></div></details>`;
  ...
  function metricsDashboard() {
    ...
    $('metrics-dashboard').innerHTML=`
      <section class="metrics-row-one">
        ...
        <article class="dashboard-panel panel-topic"><div class="panel-heading"><div><p class="eyebrow">TOPIC DISTRIBUTION</p><h2>Where work accumulates.</h2></div><div class="landscape-status"><span>CURRENT TOPICS</span><strong>${topicRows.length}</strong></div></div><p class="small">Composite activity is a descriptive count of projects, research records, evidence, notebooks, and accepted wakes touching each configured topic. It is not a quality score.</p><div class="topic-metrics">${topicChart||'<p class="empty">No topic activity yet.</p>'}</div><div class="topic-legend" aria-label="Topic distribution color key"><span><i class="topic-projects"></i>projects</span><span><i class="topic-research"></i>research</span><span><i class="topic-evidence"></i>evidence</span><span><i class="topic-notebooks"></i>notebooks</span><span><i class="topic-wakes"></i>wakes</span></div></article>
        ...
        <article class="dashboard-panel panel-provider"><p class="eyebrow">PROVIDER PRESSURE</p><h2>What it costs to get a wake.</h2><div class="dashboard-stat"><strong>${providerRequests}</strong><span>recorded HTTP requests</span></div><div class="dashboard-stat"><strong>${fallbackWakes}</strong><span>wakes using provider fallback</span></div><div class="dashboard-stat"><strong>${deferredCount}</strong><span>deferred wakes</span></div><div class="model-metrics">${modelRows||'<p class="empty">No provider data yet.</p>'}</div></article>
      </section>
...
<section class="dashboard-kpis">${card(s.version,'Durable cycles','Accepted state advances')}${card(acceptanceRate+'%','Acceptance rate',acceptedCount+' of '+completed.length+' completed wakes')}${card(handoffRate+'%','Obligation handoff',inheritedFulfilled.length+' cross-invocation fulfillments')}${card(requestsPerAccepted,'Requests / accepted','Recorded HTTP attempts ÷ accepted wakes')}${card(fallbackWakes,'Fallback wakes','More than one provider attempt')}${card(medianLatency===null?'—':medianLatency+'ms','Median provider latency','Known completed model attempts')}${card(revisedBeliefs,'Belief actions',activeBeliefs.length+' active · '+retractedBeliefs.length+' retracted')}${card(overdue,'Overdue obligations','Open commitments at or past due cycle')}</section></div>
  ...
  function blog(selected='') {
    if(selected.startsWith('topic:')){
      ...
      $('blog-content').innerHTML='<p class="topic-filter-note">Showing <strong>'+esc(topic==='reflection'?'reflection':topicLabel(topic).toLowerCase())+'</strong> · <a href="#blog">show all</a></p>'+(filtered.length?'<div class="blog-grid">'+filtered.map((post,index)=>{const invocation=s.invocations[post.created_by];const reflection=postTopic(post)==='reflection';return '<details class="blog-card record-panel'+(reflection?' blog-card-reflection':'')+'"'+(index<2?' open':'')+'><summary><div class="record-panel-meta">'+topicTag(postTopic(post),'blog',postTopic(post)==='reflection'?'reflection':topicLabel(postTopic(post)))+'<time>'+esc(fmt(invocation.time))+'</time></div><h2>'+esc(post.title)+'</h2></summary><div class="record-panel-body"><p>'+esc(post.lede)+'</p>'+(post.lens?'<blockquote>'+esc(post.lens)+'</blockquote>':'')+'<a class="text-link" href="#blog/'+encodeURIComponent(post.id)+'">Read Bob’s note →</a></div></details>';}).join('')+'</div>':'<p class="empty">No posts match this topic.</p>');
      return;
    }
    if(selected){
    ...
    $('blog-content').innerHTML=posts.length?'<div class="blog-grid">'+posts.map((post,index)=>{const invocation=s.invocations[post.created_by];const reflection=(Number(post.created_version)%10===0)&&(/reflection/i.test(String(post.id))||/reflection/i.test(String(post.title)));return '<details class="blog-card record-panel'+(reflection?' blog-card-reflection':'')+'"'+(index<2?' open':'')+'><summary><div class="record-panel-meta">'+topicTag(postTopic(post),'blog',postTopic(post)==='reflection'?'reflection':topicLabel(postTopic(post)))+'<time>'+esc(fmt(invocation.time))+'</time></div><h2>'+esc(post.title)+'</h2></summary><div class="record-panel-body"><p>'+esc(post.lede)+'</p>'+(post.lens?'<blockquote>'+esc(post.lens)+'</blockquote>':'')+'<a class="text-link" href="#blog/'+encodeURIComponent(post.id)+'">Read Bob’s note →</a></div></details>';}).join('')+'</div><p class="blog-disclosure">Bob is the public translation layer. Underneath, WAKE✳︎ is a sequence of fresh model calls working from a durable, auditable record—not a persistent person or experiencing self.</p>':'<div class="empty blog-empty"><strong>Bob has nothing worth posting yet.</strong><br>The journal still records every wake. The blog waits for something genuinely interesting.</div>';
  }
