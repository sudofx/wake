...
(() => {
  ...
  function projectCard(p,expanded=false){const stamp=panelTime(p);return `<details class="project-card record-panel"${expanded?' open':''}><summary><div class="record-panel-meta"><span class="badge">${esc(p.status.toUpperCase())}</span>${topicTag(p.domain)}${stamp?`<time>${esc(stamp)}</time>`:''}</div><h3>${esc(p.title)}</h3></summary><div class="record-panel-body"><p>${esc(p.question)}</p><div class="next-step"><span>NEXT STEP</span>${esc(p.next_step)}</div><a class="text-link" href="#projects/${encodeURIComponent(p.id)}">Follow this question →</a></div></details>`;}
  function bookCard(n,expanded=false){const stamp=panelTime(n);return `<details class="notebook-card record-panel"${expanded?' open':''}><summary><div class="record-panel-meta"><span class="eyebrow">${esc(topicName(n.domain))} / REVISION ${n.revision}</span>${topicTag(n.domain)}${stamp?`<time>${esc(stamp)}</time>`:''}</div><h3>${esc(n.title)}</h3></summary><div class="record-panel-body"><p>${esc(n.summary)}</p><a class="text-link" href="#projects/notebook:${encodeURIComponent(n.id)}">Read the notebook →</a></div></details>`;}
  function notebook(n){
