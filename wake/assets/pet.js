...
(() => {
  ...
  function projectCard(p){const stamp=panelTime(p);return `<article class="project-card record-panel"><div class="record-panel-head"><div class="record-panel-meta"><span class="badge">${esc(p.status.toUpperCase())}</span>${topicTag(p.domain)}${stamp?`<time>${esc(stamp)}</time>`:''}</div><h3><a href="#projects/${encodeURIComponent(p.id)}">${esc(p.title)}</a></h3></div><div class="record-panel-body"><p>${esc(p.question)}</p><div class="next-step"><span>NEXT STEP</span>${esc(p.next_step)}</div><a class="text-link" href="#projects/${encodeURIComponent(p.id)}">Follow this question →</a></div></article>`;}
  function bookCard(n){const stamp=panelTime(n);return `<article class="notebook-card record-panel"><div class="record-panel-head"><div class="record-panel-meta"><span class="eyebrow">${esc(topicName(n.domain))} / REVISION ${n.revision}</span>${topicTag(n.domain)}${stamp?`<time>${esc(stamp)}</time>`:''}</div><h3><a href="#projects/notebook:${encodeURIComponent(n.id)}">${esc(n.title)}</a></h3></div><div class="record-panel-body"><p>${esc(n.summary)}</p><a class="text-link" href="#projects/notebook:${encodeURIComponent(n.id)}">Read the notebook →</a></div></article>`;}
  function notebook(n){
