...
(() => {
  ...
  const topicTag=(id)=>`<a class="topic-tag" href="#projects/topic:${encodeURIComponent(id)}">${esc(topicName(id).toLowerCase())}</a>`;
  ...
  function projectCard(p){return `<article class="project-card"><div class="entry-meta"><span class="badge">${esc(topicName(p.domain))}</span><span>${esc(p.status.toUpperCase())}</span>${topicTag(p.domain)}</div><h3>${esc(p.title)}</h3><p>${esc(p.question)}</p><div class="next-step"><span>NEXT STEP</span>${esc(p.next_step)}</div><a class="text-link" href="#projects/${encodeURIComponent(p.id)}">Follow this question →</a></article>`;}
  function bookCard(n){return `<article class="notebook-card"><span class="eyebrow">${esc(topicName(n.domain))} / REVISION ${n.revision} ${topicTag(n.domain)}</span><h3>${esc(n.title)}</h3><p>${esc(n.summary)}</p><a class="text-link" href="#projects/notebook:${encodeURIComponent(n.id)}">Read the notebook →</a></article>`;}
  function notebook(n){
