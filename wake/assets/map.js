...
function button(id,showTopic=false){const n=nodes.get(id), b=el('button','node '+n.kind);b.type='button';b.dataset.node=id;b.setAttribute('aria-label',labels[n.kind]+': '+n.title);const kind=el('span','kind',labels[n.kind]);if(n.detail?.status){const status=el('small','status-tag '+n.detail.status,String(n.detail.status).replaceAll('_',' '));kind.append(status)}const topic=topicFor(n);if(topic){const tag=el('small','topic-tag',topic.replaceAll('_',' ').toLowerCase());tag.dataset.topic=topic;tag.style.setProperty('--topic-color',topicColors[topic]||'var(--cyan)');kind.append(tag)}const stamp=timeLabel(timeFor(n));if(stamp)kind.append(el('small','node-time',stamp));b.append(kind);b.append(el('span','node-title',n.title));b.addEventListener('click',()=>select(id));b.addEventListener('pointerenter',()=>emphasis(id));b.addEventListener('pointerleave',()=>emphasis());return b}
...
function showDetails(id){const n=nodes.get(id);details.replaceChildren();details.classList.add('active');const heading=el('div','detail-heading');const browse=el('button','browse-artifacts','Browse artifacts');browse.addEventListener('click',()=>{details.classList.remove('active');constellation.querySelector('button')?.focus({preventScroll:true})});heading.append(browse);const close=el('button','','Close');close.addEventListener('click',clear);heading.append(close);details.append(heading);const meta=el('div','record-panel-meta map-detail-meta');meta.append(el('span','record-type',labels[n.kind]));if(n.detail?.status){const holder=el('span','record-status'),chip=el('span','badge '+String(n.detail.status).toLowerCase().replace(/[^a-z0-9_-]+/g,'-'),String(n.detail.status).replaceAll('_',' ').toUpperCase());holder.append(chip);meta.append(holder)}else meta.append(el('span','record-status',''));const domain=topicFor(n);if(domain){const topics=el('span','record-topics'),tag=el('span','topic-tag',domain.replaceAll('_',' ').toLowerCase());tag.style.setProperty('--topic-color',topicColors[domain]||'var(--cyan)');topics.append(tag);meta.append(topics)}else meta.append(el('span','record-topics',''));const stamp=timeLabel(timeFor(n));if(stamp)meta.append(el('time','',stamp));details.append(meta);const title=el('h2','',n.title);title.tabIndex=-1;details.append(title);
