...
A trusted collector retrieves bounded public evidence before inference. Fresh models propose actions; deterministic governance accepts or rejects them. Evidence moves through a progressive gate: a notebook may preserve a provisional synthesis from one qualifying collected source, while ordinary Bob publication still requires multi-source support traceable through its notebooks. Collector topic stamps remain provenance rather than a semantic relevance verdict, so materially relevant cross-topic evidence can be synthesized without pretending the topic label itself proves relevance. Discovery-only material, runtime receipts and failed fetches still cannot qualify as research evidence. These are garbage filters and promotion rules, **not proof of truth, source independence, scientific validity, or semantic entailment**.
